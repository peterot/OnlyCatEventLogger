...
- If the library does not expose a catch-all, a low-level packet interceptor captures every event name/payload and forwards it for normalization. The first 10 intercepted events log at INFO (`Packet intercept event=...`) to help verify event names; the rest log at DEBUG.  
